(define (domain simple)
    ...
    (:predicates
        ...
        (person_location_unknown ?p - person)
    ...
    (:action find_person
        :parameters (?p - person ?ro - room)
        :precondition (and
            (person_location_unknown ?p)
        )
        :observe
        (person_at ?p ?ro)
    )

    (:durative-action find_person_durative
        :parameters (?p - person ?ro - room)
        :duration ( = ?duration 5)
        :condition (and
            (at start(person_location_unknown ?p))
        )
        :observe
        (person_at ?p ?ro)
    )
