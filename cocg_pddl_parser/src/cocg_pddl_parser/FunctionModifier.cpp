...
FunctionModifier::FunctionModifier(const std::string &name, Function *f,
                                   const IntVec &p)
    : name(name),
      modifiedGround(0),
      modifierExpr(new FunctionExpression(new Ground(f, p))) {}

FunctionModifier::FunctionModifier(const std::string &name,
...
void FunctionModifier::PDDLPrint(std::ostream &s, unsigned indent,
...
cocg_ast::Node::SharedPtr FunctionModifier::getTree(
  ...
  node->node_type = cocg_ast::Node::FUNCTION_MODIFIER;
  node->modifier_type = getFunModType(name);
  node->node_id = tree.nodes.size();
  tree.nodes.push_back(*node);

  if (modifiedGround) {
    cocg_ast::Node::SharedPtr child = modifiedGround->getTree(tree, d, replace);
    tree.nodes[node->node_id].children.push_back(child->node_id);
  } else {
    std::cerr << "function modifier for total-cost not supported" << std::endl;
  }

  cocg_ast::Node::SharedPtr child = modifierExpr->getTree(tree, d, replace);
  tree.nodes[node->node_id].children.push_back(child->node_id);

  return node;
}

void FunctionModifier::parse(Stringreader &f, TokenStruct<std::string> &ts,
                             Domain &d) {
  f.next();
