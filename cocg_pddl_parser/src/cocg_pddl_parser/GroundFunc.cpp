...
template <>
void GroundFunc<double>::PDDLPrint(std::ostream &s, unsigned indent,
...
template <>
void GroundFunc<int>::PDDLPrint(std::ostream &s, unsigned indent,
                                const TokenStruct<std::string> &ts,
                                const Domain &d) const {
  tabindent(s, indent);
  s << "( = ";
...
template <>
void GroundFunc<double>::parse(Stringreader &f, TokenStruct<std::string> &ts,
                               Domain &d) {
...
template <>
void GroundFunc<int>::parse(Stringreader &f, TokenStruct<std::string> &ts,
                            Domain &d) {
  TypeGround::parse(f, ts, d);
  ...
  std::pair<bool, unsigned> p =
      d.types[((Function *)lifted)->returnType]->parseObject(s);
  if (p.first) {
    value = p.second;
  } else {
    std::pair<bool, int> q =
        d.types[((Function *)lifted)->returnType]->parseConstant(s);
    if (q.first) {
      value = q.second;
    } else {
      f.tokenExit(s);
