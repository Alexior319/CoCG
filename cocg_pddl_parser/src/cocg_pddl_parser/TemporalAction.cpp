...
Expression *TemporalAction::parseDuration(Stringreader &f,
                                          TokenStruct<std::string> &ts,
                                          Domain &d) {
  return createExpression(f, ts, d);
}

void TemporalAction::printCondition(std::ostream &s,
                                    const TokenStruct<std::string> &ts,
                                    const Domain &d, const std::string &t,
                                    And *a) const {
...
void TemporalAction::PDDLPrint(std::ostream &s, unsigned indent,
                               const TokenStruct<std::string> &ts,
...
cocg_ast::Node::SharedPtr TemporalAction::getTree(
    cocg_ast::Tree &tree, const Domain &d,
    const std::vector<std::string> &replace) const {
  throw UnsupportedConstruct("TemporalAction");
}

void TemporalAction::parseCondition(Stringreader &f,
                                    TokenStruct<std::string> &ts, Domain &d,
                                    And *a) {
  f.next();
  f.assert_token("(");
...
void TemporalAction::parse(Stringreader &f, TokenStruct<std::string> &ts,
                           Domain &d) {
  f.next();
