...
void FunctionExpression::PDDLPrint(std::ostream &s, unsigned indent,
  ...
  for (unsigned i = 0; i < psize; ++i) {
    if (ts.size() && fun->params[i] >= 0 &&
        (unsigned)fun->params[i] < ts.size()) {
      s << " " << ts[fun->params[i]];
    } else if (fun->params[i] >= 0 && (unsigned)fun->params[i] >= ts.size()) {
      s << " ?" << fun->params[i];
    } else {
      // It is a constant parameter for the function
      s << " ";
      constants[i]->PDDLPrint(s, indent, ts, d);
    }
  }
  s << " )";
}
...
double FunctionExpression::evaluate(Instance &ins, const StringVec &par) {
  ...
  for (unsigned i = 0; i < v.size(); ++i) {
    std::pair<bool, unsigned> p =
        ins.d.types[c->params[i]]->parseObject(par[fun->params[i]]);
    if (p.first) {
      v[i] = p.second;
    } else {
      std::pair<bool, int> q =
          ins.d.types[c->params[i]]->parseConstant(par[fun->params[i]]);
      if (q.first) {
        v[i] = q.second;
      } else {
        return 1;
...
void ConstExpression::PDDLPrint(std::ostream &s, unsigned indent,
                                const TokenStruct<std::string> &ts,
                                const Domain &d) const {
  s << d.types[tid]->constants[constant];
}

Expression *createExpression(Stringreader &f, TokenStruct<std::string> &ts,
                             Domain &d) {
  f.next();
