...
#include "cocg_pddl_parser/EitherType.h"
#include "cocg_pddl_parser/Exists.h"
#include "cocg_pddl_parser/Forall.h"
#include "cocg_pddl_parser/Function.h"
#include "cocg_pddl_parser/FunctionModifier.h"
#include "cocg_pddl_parser/GroundFunc.h"
#include "cocg_pddl_parser/Not.h"
#include "cocg_pddl_parser/Oneof.h"
#include "cocg_pddl_parser/Or.h"
#include "cocg_pddl_parser/Task.h"
#include "cocg_pddl_parser/TemporalAction.h"
#include "cocg_pddl_parser/Unknown.h"
#include "cocg_pddl_parser/When.h"
...
class Domain {
 public:
  ...
  bool strips, adl, condeffects;  // whether domain is strips, adl and/or has
                                  // conditional effects
  bool typed, cons, costs;  // whether domain is typed, has constants, has costs
  bool temp, nondet, neg,
      disj;  // whether domain is temporal, is non-deterministic, has negative
             // precons, has disjunctive preconditions
  ...
  Domain()
      : equality(false),
        ...
        fluents(false),
        derivedpred(false) {
    types.insert(new Type("object"));  // Type 0 is always "object", whether the
                                       // domain is typed or not
  }

  Domain(const std::string& s) : Domain() { parse(s); }
  ...
  virtual bool parseBlock(const std::string& t, Stringreader& f) {
    if (t == "requirements") {
      ...
      parseAction(f);
    } else if (t == "durative-action") {
      parseDurativeAction(f);
    } else if (t == "derived") {
      parseDerived(f);
    }
  ...
  virtual bool parseRequirement(const std::string& s) {
    if (s == "strips") {
      ...
      adl = true;
    } else if (s == "negative-preconditions") {
      neg = true;
    } else if (s == "conditional-effects") {
      condeffects = true;
    } else if (s == "typing") {
      typed = true;
    } else if (s == "action-cost") {
      costs = true;
    } else if (s == "equality") {
      equality = true;
    } else if (s == "durative-actions") {
      temp = true;
    } else if (s == "non-deterministic") {
      nondet = true;
    } else if (s == "universal-preconditions") {
      universal = true;
    } else if (s == "fluents") {
      fluents = true;
    } else if (s == "disjunctive-preconditions") {
      disj = true;
    } else if (s == "derived-predicates") {
      derivedpred = true;
    } else {
  ...
  void parseTypes(Stringreader& f) {
    ...
    //		if this makes it in, probably need to define new subclass of
    //Type 		if ( costs ) insert( new Type( "NUMBER" ), tmap, types );
    ...
    for (unsigned i = 0; i < ts.size(); ++i) {
      if (std::find(types.types.begin(), types.types.end(), ts.types[i]) ==
          types.types.end()) {
        if (ts.types[i].size()) {
  ...
  Lifted* createPredicate(const std::string& name,
                          const StringVec& params = StringVec()) {
  ...
  Lifted* createFunction(const std::string& name, int type,
                         const StringVec& params = StringVec()) {
  ...
  void addPre(bool neg, const std::string& act, const std::string& pred,
              const IntVec& params = IntVec()) {
    Action* action = actions.get(act);
    if (action->pre == 0) {
      action->pre = new And;
    }
    ...
    if (neg) {
      ...
      a->add(ground(pred, params));
    }
  }

  // Add an "or" precondition to the action with name "act"
  void addOrPre(const std::string& act, const std::string& pred1,
                const std::string& pred2, const IntVec& params1 = IntVec(),
                const IntVec& params2 = IntVec()) {
    Or* o = new Or;
  ...
  void addEff(bool neg, const std::string& act, const std::string& pred,
              const IntVec& params = IntVec()) {
    Action* action = actions.get(act);
    if (action->eff == 0) {
      action->eff = new And;
  ...
  void addCost(const std::string& act, const std::string& func,
               const IntVec& params = IntVec()) {
    Action* action = actions.get(act);
    if (action->eff == 0) {
      action->eff = new And;
  ...
  Ground* ground(const std::string& name, const IntVec& params = IntVec()) {
    if (preds.index(name) < 0) {
      ...
      std::cout << " failed since the predicate " << name
                << " does not exist!\n";
      std::exit(1);
    }
  ...
  friend std::ostream& operator<<(std::ostream& os, const Domain& o) {
    return o.print(os);
  }
  ...
  virtual std::ostream& print_addtional_blocks(std::ostream& os) const {
    return os;
  }
